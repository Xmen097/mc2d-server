...
var smallRecipes=[[2, 11, 4],
				 ...
				 [52, undefined, undefined, 52, 34, 1]]
...
function onMapEdit(data) {
	if(parseInt(data.block) == -1 && map[parseInt(data.x)][parseInt(data.y)] && items[map[parseInt(data.x)][parseInt(data.y)]] && playerById(this.id).inventory.hotbar[parseInt(data.active)]) {
		...
		util.log(arseInt(data.block) == -1 && map[parseInt(data.x)][parseInt(data.y)] && items[map[parseInt(data.x)][parseInt(data.y)]] && playerById(this.id).inventory.hotbar[parseInt(data.active)]);
		return;
	}
